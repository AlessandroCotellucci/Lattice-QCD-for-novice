...
def S(j,x): # harm. osc. S
    N=len(x)
    a=1/2
    sec_deri=ones(N, 'double')
    for k in range(0,N):
        sec_deri[k]=sec_der(x,k)
    return a*(a*x[j]**2/2-(1/2)*x[j]*(sec_der(x,j)-((a**2)/12)*sec_der(sec_deri,j)))
...
for k in range(1,N_stat): #Inizialization of the N_stat-1 simulations
    simulations_G[k]=bootstrap(G)
